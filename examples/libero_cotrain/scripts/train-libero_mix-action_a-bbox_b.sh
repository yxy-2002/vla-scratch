...
# Allow overrides like: DL_WORKERS=8 PREFETCH=4 NPROCS=2 bash train-libero_mix-action_a-bbox_b.sh
for arg in "$@"; do
    if [[ "$arg" == *=* ]]; then
        export "$arg"
    fi
...
echo "train-libero_mix-action_a-bbox_b"
